...
export default function StickyHeadTable() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:4000/grade6-details');
  ...
  const saveEditedRecord = async () => {
    try {
      ...
      await axios.put(`http://localhost:4000/grade6-details/${editedRow.stockId}`, editedRow);
      setIsEditing(false);
      setEditedRow(null);
  ...
  const deleteRecord = async row => {
    try {
      const { stockId } = row;
      await axios.delete(`http://localhost:4000/grade6-details/${stockId}`);
