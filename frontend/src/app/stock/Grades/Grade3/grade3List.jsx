...
export default function StickyHeadTable() {
  ...
  const saveEditedRecord = async () => {
    try {
      ...
      await axios.put(`http://localhost:4000/grade3-details/${editedRow.grade3}`, editedRow);
      setIsEditing(false);
      setEditedRow(null);
      console.log('Record edited successfully.');
  
      // Update the data state with the edited row
      setData(prevData => prevData.map(item => (item.grade3 === editedRow.grade3 ? editedRow : item)));
    } catch (error) {
  ...
  const deleteRecord = async row => {
    try {
      const { grade3 } = row;
      await axios.delete(`http://localhost:4000/grade3-details/${grade3}`);
      setData(prevData => prevData.filter(item => item.grade3 !== grade3));
                ...
                .map((row, index) => (
                  <TableRow hover role="checkbox" tabIndex={-1} key={index}>
                    <TableCell align="center" style={{ minWidth: 70 }}>
                      {isEditing && editedRow.grade3 === row.grade3 ? (
                        <input
                          type="date"
